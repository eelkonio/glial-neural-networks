#!/usr/bin/env python3
"""Run the full Step 14 experiment.

50 epochs × 3 seeds × 6 conditions with timestamp logging.

Expected runtime: ~3-4 hours total on CPU.
...
def main():
    print("=" * 70)
    print("STEP 14: PREDICTIVE CODING + BCM — FULL EXPERIMENT")
    print("=" * 70)
    ...
    print(f"Expected runtime: ~3-4 hours total")
    print("=" * 70)
    ...
    for cond_idx, condition in enumerate(conditions):
        print(f"\n{'─' * 70}")
        print(f"CONDITION {cond_idx + 1}/{len(conditions)}: {condition.name}")
    ...
    for condition in conditions:
        ...
        std_acc = (sum((a - mean_acc) ** 2 for a in accs) / max(len(accs) - 1, 1)) ** 0.5 if len(accs) > 1 else 0
        print(f"  {condition.name:30s}: {mean_acc*100:.2f}% ± {std_acc*100:.2f}% "
              f"(seeds: {[f'{a*100:.1f}%' for a in accs]})")

    # Save results
    output = {
        "experiment": "predictive_coding_bcm_full",
